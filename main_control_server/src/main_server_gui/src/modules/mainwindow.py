...
class MainWindow(QtWidgets.QMainWindow):
    ...
    outbound_list_signal = pyqtSignal(list)  # 새로운 signal 정의
    #8시 알려라~ node에~
    ...
    def __init__(self, username=''):
        ...
        self.outbound_list_signal.connect(self.display_outbound_list)  # 추가
    ...
    def show_inventory_management(self):
        if not self.username:
            QtWidgets.QMessageBox.warning(self, 'Error', '로그인 후에 진행해 주세요.')
            return
        data = self.fetch_inventory_table_data()
        if data:
            self.populate_table_widget(data, 'Inventory')
        else:
            QtWidgets.QMessageBox.warning(self, 'Error', 'No data found in Inventory')

    def fetch_inventory_table_data(self):
        db_instance = get_mysql_connection()
        if db_instance:
            try:
                db_instance.cursor.execute("SELECT * FROM Inventory")
    ...
    def show_robotstate_management(self):
        ...
        self.close()
# Outbound 관련 매써드
    def display_outbound_list(self, outbound_list):
        self.update_outbound_list(outbound_list)
        if self.outbound_management_active:
            self.show_outbound_management()

    def update_outbound_list(self, outbound_list):  # 초기 리스트 update
        db_instance = get_mysql_connection()
        if db_instance:
            try:
                db_instance.cursor.execute("SET FOREIGN_KEY_CHECKS=0")
                db_instance.cursor.execute("DELETE FROM Outbound_Manager")    # 여기서 DB 테이블 삭제
                db_instance.cursor.execute("SET FOREIGN_KEY_CHECKS=1")
                for idx, item in enumerate(outbound_list, start=1):
                    insert_query = ("INSERT INTO Outbound_Manager (No, Product_Code, Product_Name, Warehouse, Rack, Cell, Receiving_Quantity, Status) "
                                    "VALUES (%s, %s, %s, %s, %s, %s, %s, %s)")
                    data = (idx, item['item_id'], item['name'], item['warehouse'], item['rack'], item['cell'], item['quantity'], item['status'])
                    db_instance.cursor.execute(insert_query, data)
                db_instance.conn.commit()
    ...
    def fetch_outbound_table_data(self):
        ...
        if db_instance:
            try:
                db_instance.cursor.execute("SELECT * FROM Outbound_Manager")
                data = db_instance.cursor.fetchall()
                db_instance.disConnection()
                return data
            except con.Error as err:
