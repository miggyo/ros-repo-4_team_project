...
from task_manager.msg import SendLightOffResults
...
class RobotTaskClient(Node):
    ...
    def send_rack_list(self,task):
        msg = RackList()
        msg.rack_list = task.rack_list  # new 0807 해당 로봇의 rack_list보내기

        if task.task_assignment == "입고":                                                  # new 0806                                
            msg.scenario = False
        else:
            msg.scenario = True
    ...
    def send_light_off_callback(self, msg):                                                                                        # new 0805
    ...
    def get_result_callback(self, msg):
        ...
        if result == "completed":                                                                                       # new 0805
            ...
            ####################### & robotstatewindow한테 랙 정보 업데이트 하라고 보내야 함 ##############
            ...
            if not task.all_tasks_completed():  # new 0807
                ...
                timeout = 600  # 타임아웃 시간을 600초로 설정                                                                   # new 0805
                ...
                if task.LIGHT_OFF_COMPLETE:  # new 0807 서버에서 받는신호~ 
                    #### send that LIGHT OFF SUCCEESS ### 
                    light_off_msg = Bool()
                    self.num += 1
                    light_off_msg.data = self.LIGHT_OFF_COMPLETE

                if task.robot_name == "Robo1": #로봇한테 보내는신호
            ...
            #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                self.check_robot_state(task)  # new 0807                                                                                # new 0804

    ############################## 여기서 estimated_completion_time response 업데이트 ################
    def update_estimated_completion_time(self, task):  # new 0807                                                      # new 0801                  
        ...
        query = f"""
            ...
            SET Estimated_Completion_Time = Estimated_Completion_Time - 1,
                Battery_Status = CONCAT(CAST(TRUNCATE(CAST(SUBSTRING(Battery_Status, 1, LENGTH(Battery_Status) - 1) AS DECIMAL(5, 2)) - 10, 0) AS CHAR), '%')
    ...
    def send_task_complete_results(self, task):  # new 0807
        ...
        # Task Manager 및 Robot_State_Window 한테 publishing
    ...
    def check_robot_state(self, task):  # new 0807                                                                   # new 0804
        ## 1. 배터리 상태 체크 --> '20%' 이하면 --> 
